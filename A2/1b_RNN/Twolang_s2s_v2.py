...
Also needs one more thing, which is to make sure for the 2lang that the English is the same input for both

So need subset of the lines
...
class data_prep():
    def __init__(self, txtfile, de_txtfile, random_sample_flag, num_samples=10000):
        assert isinstance(random_sample_flag, bool), "flag should be boolean"
        self.frdata_path = txtfile
        self.dedata_path = de_txtfile
        ...
        self.frtarget_token_index = 'undef; run tokeniser function'
        self.detarget_token_index = 'undef; run tokeniser function'
        self.encoder_input_data = 'undef; run tokeniser function'
        self.frdecoder_input_data = 'undef; run tokeniser function'
        self.frdecoder_target_data = 'undef; run tokeniser function'
        self.dedecoder_input_data = 'undef; run tokeniser function'
        self.dedecoder_target_data = 'undef; run tokeniser function'
    ...
    def load_w2v(self):
        ...
        self.frtarget_token_index = w2v.load_word2vec_format(
            fr_path, binary=True)
        self.detarget_token_index = w2v.load_word2vec_format(
            de_path, binary=True)
    ...
    def de_line_preproc(self, line):
        ...
        for word in target_words:
            if word == '':
                continue
            if word not in self.detarget_token_index.vocab:
                skip_line = 2
                unk_line_targets.append((word, target_words))
            else:
                vec = self.detarget_token_index.word_vec(word)
        ...
        if skip_line == 0:
            ...
            self.detarget_texts.append("\t" + target_text + "\n")
    ...
    def fr_line_preproc(self, line):
        ...
        for word in target_words:
            ...
            if word == '':
                continue
            if word not in self.frtarget_token_index.vocab:
                skip_line = 2
                unk_line_targets.append((word, target_words))
            else:
                vec = self.frtarget_token_index.word_vec(word)
                vec = np.pad(vec, (2, 0), 'constant')
                target_vecs = np.vstack((target_vecs, vec))
        start_of_line = np.append([1, 0], np.zeros(200))
        end_of_line = np.append([0, 1], np.zeros(200))
        ...
        if skip_line == 0:
            self.input_texts.append(input_text)
            self.frtarget_texts.append("\t" + target_text + "\n")

        return skip_line, input_vecs, target_vecs, unk_line_inputs, \
               unk_line_targets

    def fr_as_wrdvec(self):
        np.random.seed(1)
        ...
        with open(self.frdata_path, 'r', encoding='utf-8') as f:
            lines = f.read().split('\n')
        ...
        for line in sample:
            skip, input_vecs, target_vecs, unk_in, unk_out = self.fr_line_preproc(line)
            if skip > 0:
                continue
        ...
        print("French Tar: {} In: {}".format(tar_max_len, in_max_len))

        n = len(in_vec_list)
        self.encoder_input_data = np.zeros((n, in_max_len, 300))
        self.frdecoder_input_data = np.zeros((n, tar_max_len, 202))
        self.frdecoder_target_data = np.zeros((n, tar_max_len, 202))
        for i, (in_arr, tar_arr) in enumerate(zip(in_vec_list, tar_vec_list)):
            ...
            self.frdecoder_input_data[i, :tar_len, :] = tar_arr
            self.frdecoder_target_data[i, :tar_len-1, :] = tar_arr[1:, :]
        # french word vecs are in the range of +- 1.35, normalise for use with
        # tanh activation function
        self.frdecoder_input_data /= 1.35
        self.frdecoder_target_data /= 1.35


    def de_as_wrdvec(self):
        np.random.seed(1)
        self.embed_type = "wordvec"
        self.load_w2v()
        if self.target_lang == 'de':
        ...
        with open(self.dedata_path, 'r', encoding='utf-8') as f:
            de_lines = f.read().split('\n')
        sample_sz = min(self.num_samples, len(de_lines) - 1)
        print("German Lines: {}".format(len(de_lines)))
        if self.random_sample:
            sample = np.random.choice(de_lines, sample_sz, False)
        else:
            sample = de_lines[:sample_sz]
        for line in sample:
            skip, input_vecs, target_vecs, unk_in, unk_out = self.de_line_preproc(line)
            if skip > 0:
                continue
            de_in_vec_list.append(input_vecs)
            de_tar_vec_list.append(target_vecs)
            de_in_max_len = max((de_in_max_len, input_vecs.shape[0]))
            de_tar_max_len = max((de_tar_max_len, target_vecs.shape[0]))
        print("German Tar: {} In: {}".format(de_tar_max_len, de_in_max_len))

        n = len(de_in_vec_list)
        self.encoder_input_data = np.zeros((n, de_in_max_len, 300))
        self.dedecoder_input_data = np.zeros((n, de_tar_max_len, 202))
        self.dedecoder_target_data = np.zeros((n, de_tar_max_len, 202))
        for i, (in_arr, tar_arr) in enumerate(zip(de_in_vec_list, de_tar_vec_list)):
            in_len = in_arr.shape[0]
            tar_len = tar_arr.shape[0]
            self.encoder_input_data[i, :in_len, :] = in_arr
            self.dedecoder_input_data[i, :tar_len, :] = tar_arr
            self.dedecoder_target_data[i, :tar_len-1, :] = tar_arr[1:, :]
        # french word vecs are in the range of +- 1.35, normalise for use with
        # tanh activation function
        self.dedecoder_input_data /= 1.35
        self.dedecoder_target_data /= 1.35
    ...
    def as_wrdvec(self):
        ...
        Now the combo part, take the longer vector of the two
        
        And the larger example space, not hugely different, but does matter
        
        """

        n = max(len(de_in_vec_list), len(in_vec_list))
        in_max_len = max(in_max_len, de_in_max_len)
        tar_max_len = max(tar_max_len, de_tar_max_len)
        self.encoder_input_data = np.zeros((n, in_max_len, 300))
        self.dedecoder_input_data = np.zeros((n, tar_max_len, 202))
        self.dedecoder_target_data = np.zeros((n, tar_max_len, 202))
        for i, (in_arr, tar_arr) in enumerate(zip(de_in_vec_list, de_tar_vec_list)):
            in_len = in_arr.shape[0]
            tar_len = tar_arr.shape[0]
            self.encoder_input_data[i, :in_len, :] = in_arr
...
class seq2seq():
    def __init__(self, data_prep):
        self.model = None
        self.enc_model = None
        self.fr_dec_model = None
        self.de_dec_model = None
        self.data = data_prep
        self.act_func = None
        self.loss_func = None
        self.filename = None
    ...
    def reverse(self, act_func, loss_func, latent_dim=256):
        ...
        self.twolang(act_func, loss_func, latent_dim)
    ...
    def twolang(self, act_func, loss_func, latent_dim=256):
        ...
        frdecoder_inputs = Input(shape=(None,
                                        self.data.frdecoder_input_data.shape[2]))
        ...
        frdecoder_outputs, _, _ = frdecoder_l2(frdecoder_outputs,
                                               initial_state=[state_h2, state_c2])
        frdecoder_dense = Dense(self.data.frdecoder_input_data.shape[2],
                                activation=act_func)
        frdecoder_outputs = frdecoder_dense(frdecoder_outputs)

        dedecoder_inputs = Input(shape=(None,
                                        self.data.dedecoder_input_data.shape[2]))
        ...
        dedecoder_outputs, _, _ = dedecoder_l2(dedecoder_outputs,
                                               initial_state=[state_h2, state_c2])
        dedecoder_dense = Dense(self.data.dedecoder_input_data.shape[2],
                                activation=act_func)
        dedecoder_outputs = dedecoder_dense(dedecoder_outputs)
        ...
        frdecoder_state_input_h = Input(shape=(latent_dim,))
        frdecoder_state_input_c = Input(shape=(latent_dim,))
        frdecoder_state_input_h2 = Input(shape=(latent_dim,))
        frdecoder_state_input_c2 = Input(shape=(latent_dim,))
        dedecoder_state_input_h = Input(shape=(latent_dim,))
        dedecoder_state_input_c = Input(shape=(latent_dim,))
        dedecoder_state_input_h2 = Input(shape=(latent_dim,))
        dedecoder_state_input_c2 = Input(shape=(latent_dim,))
        frdecoder_states_inputs = [frdecoder_state_input_h, frdecoder_state_input_c,
                                 frdecoder_state_input_h2, frdecoder_state_input_c2]
        dedecoder_states_inputs = [dedecoder_state_input_h, dedecoder_state_input_c,
                                   dedecoder_state_input_h2, dedecoder_state_input_c2]
        # French Part
        frdecoder_outputs, frdecoder_state_h1, frdecoder_state_c1 = \
            frdecoder_l1(frdecoder_inputs, initial_state=frdecoder_states_inputs[:2])
        frdecoder_outputs, frdecoder_state_h2, frdecoder_state_c2 = \
            frdecoder_l2(frdecoder_outputs, initial_state=frdecoder_states_inputs[-2:])
        frdecoder_states = [frdecoder_state_h1, frdecoder_state_c1,
                          frdecoder_state_h2, frdecoder_state_c2]
        frdecoder_outputs = frdecoder_dense(frdecoder_outputs)
        # German Part
        dedecoder_outputs, dedecoder_state_h1, dedecoder_state_c1 = \
            dedecoder_l1(dedecoder_inputs, initial_state=dedecoder_states_inputs[:2])
        dedecoder_outputs, dedecoder_state_h2, dedecoder_state_c2 = \
            dedecoder_l2(dedecoder_outputs, initial_state=dedecoder_states_inputs[-2:])
        dedecoder_states = [dedecoder_state_h1, dedecoder_state_c1,
                          dedecoder_state_h2, dedecoder_state_c2]
        dedecoder_outputs = dedecoder_dense(dedecoder_outputs)

        self.fr_dec_model = Model([frdecoder_inputs] + frdecoder_states_inputs,
                               [frdecoder_outputs] + frdecoder_states)

        self.de_dec_model = Model([dedecoder_inputs] + dedecoder_states_inputs,
                                  [dedecoder_outputs] + dedecoder_states)

    # Run training
    def fit(self, pre, batch_sz=64, epochs=100):
        print(self.model.summary())
        early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=10, verbose=0, mode='auto', baseline=None, restore_best_weights=False)
        self.model.fit([self.data.encoder_input_data,
                        self.data.frdecoder_input_data, self.data.dedecoder_input_data],
                       [self.data.frdecoder_target_data, self.data.dedecoder_target_data],
                       batch_size=batch_sz,
                       epochs=epochs,
    ...
    def decode_sequence(self, input_seq):
        ...
        n = input_seq.shape[0]
        fr_target_seq = np.zeros((n, 1, self.data.frdecoder_input_data.shape[2]))
        de_target_seq = np.zeros((n, 1, self.data.dedecoder_input_data.shape[2]))
        ...
        fr_decoded_sentences = ["" for i in range(n)]
        for i in range(self.data.frdecoder_input_data.shape[1]):
            output_tokens, h1, c1, h2, c2 = self.fr_dec_model.predict(
                [fr_target_seq] + states_value)
            fr_target_seq.fill(0)
            ...
            if self.data.embed_type == 'char':
                ...
                fr_target_seq[list(range(n)), 0, sampled_token_index] = 1.
                decoded_sentences = [a + decode_index(b) for (a, b) in
                                     zip(decoded_sentences, sampled_token_index)]
            elif self.data.embed_type == 'wordvec':
                for j in range(n):
                    pred_vec = output_tokens[j, :, :]
                    sim_word = self.data.frtarget_token_index.similar_by_vector(
                        pred_vec[0, 2:], 1)[0][0]
                    sim_vec = self.data.frtarget_token_index.word_vec(sim_word)
                    ...
                    fr_target_seq[j, :, :] = vec_list[arg]
                    fr_decoded_sentences[j] += word_list[arg]
        for k, sen in enumerate(fr_decoded_sentences):
            sen = sen[1:]
            fr_decoded_sentences[k] = sub("\n.*", "", sen, flags=DOTALL)

        de_decoded_sentences = ["" for i in range(n)]
        for i in range(self.data.dedecoder_input_data.shape[1]):
            output_tokens, h1, c1, h2, c2 = self.de_dec_model.predict(
                [de_target_seq] + states_value)
            de_target_seq.fill(0)
            states_value = [h1, c1, h2, c2]
            ...
            if self.data.embed_type == 'char':
                ...
                sampled_token_index = list(
                    np.argmax(output_tokens, 2).flatten())
                de_target_seq[list(range(n)), 0, sampled_token_index] = 1.
                decoded_sentences = [a + decode_index(b) for (a, b) in
                                     zip(decoded_sentences, sampled_token_index)]
            elif self.data.embed_type == 'wordvec':
                for j in range(n):
                    pred_vec = output_tokens[j, :, :]
                    sim_word = self.data.detarget_token_index.similar_by_vector(
                        pred_vec[0, 2:], 1)[0][0]
                    sim_vec = self.data.detarget_token_index.word_vec(sim_word)
                    sim_vec = np.pad(sim_vec, (2, 0), 'constant').reshape(1, -1)
                    vec_list = [sim_vec, vec_sol, vec_eol]
                    word_list = [" " + sim_word, "\t", "\n"]
                    arg = np.argmin(
                        [cosine_distances(pred_vec, v) for v in vec_list])
                    de_target_seq[j, :, :] = vec_list[arg]
                    de_decoded_sentences[j] += word_list[arg]
        for k, sen in enumerate(de_decoded_sentences):
            sen = sen[1:]
            de_decoded_sentences[k] = sub("\n.*", "", sen, flags=DOTALL)

        return fr_decoded_sentences, de_decoded_sentences, n
    ...
    def train_acc(self, beg=0, end=np.inf):
        ...
        decorrect = 0
        incorrects = []
        fr_preds, de_preds, n = self.decode_sequence(self.data.encoder_input_data[beg:end])
        for (truth, pred) in zip(self.data.frtarget_texts, fr_preds):
            if truth[1:-1] == pred:
                frcorrect += 1
            else:
                incorrects.append((truth[1:-1], pred))
        print("French Accuracy:", frcorrect/n)
        for (truth, pred) in zip(self.data.detarget_texts, de_preds):
            if truth[1:-1] == pred:
                decorrect += 1
            else:
                incorrects.append((truth[1:-1], pred))
        print("German Accuracy:", decorrect/n)
        print("Overall Accuracy:", (decorrect+frcorrect)/(2*n))
        with open(self.filename + '.list', 'wb') as f:
            dump([(decorrect+frcorrect)/(2*n)] + incorrects, f)

        return (decorrect+frcorrect)/(2*n), incorrects
